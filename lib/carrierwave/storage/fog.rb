...
module CarrierWave
  module Storage
    ...
    # [:fog_force_path_for_aws]           (optional) force use of the path rather than subdomain
    #   for AWS generated url
    #
    #
        ...
        def public_url
          if host = @uploader.fog_host
            ...
            when 'AWS'
              ...
              if not @uploader.fog_force_path_for_aws and @uploader.fog_directory.to_s =~ /^(?:[a-z]|\d(?!\d{0,2}(?:\d{1,3}){3}$))(?:[a-z0-9]|(?![\-])|\-(?![\.])){1,61}[a-z0-9]$/
